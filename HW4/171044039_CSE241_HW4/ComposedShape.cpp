...
namespace shape_ozmekik
{
	...
	const Polygon ComposedShape::operator[](unsigned int idx)const{return shape[idx];} 
	Polygon ComposedShape::operator[](unsigned int idx){return shape[idx];}


	std::ostream& operator<<(std::ostream& outfile,const ComposedShape& c)
	{	
		...
		for(unsigned int i=0;i<c.shape.size();++i)
			outfile<<c.shape[i];
	...
	const std::vector<Polygon> ComposedShape::fitRinR()const
	{ 
		Rectangle r(r2); // Temp small shape which we will work on.
		std::vector<Polygon> _shape; // Small shapes. 
	...
	const std::vector<Polygon> ComposedShape::fitRinC()const
	{
	...
	const std::vector<Polygon> ComposedShape::fitRinT()const
	{
	...
	const std::vector<Polygon> ComposedShape::fitCinR()const
	{
		std::vector<Polygon> _shape;
		Circle c(c2);
		double x,y;
	...
	const std::vector<Polygon> ComposedShape::fitCinC()const
	{
		Rectangle inner_r;
	...
	const std::vector<Polygon> ComposedShape::fitCinT()const
	{
		Rectangle inner_r;
		Circle c(c2);
	...
	const std::vector<Polygon> ComposedShape::fitTinR()const
	{
		Triangle t(t2); 
		std::vector<Polygon> _shape;
		double x,y;
	...
	const std::vector<Polygon> ComposedShape::fitTinC()const
	{
		Rectangle inner_r;
		Triangle t(t2);
		Circle c(c1);
	...
	const std::vector<Polygon> ComposedShape::fitTinT()const
	{
